...
class QuanLyHoSo:
    ...
    def dang_ky(self, username, password, ten_hien_thi):
        ...
        try:
            ...
            cursor.execute("INSERT INTO NguoiDung (username, password, ten_hien_thi, diem_cao_nhat, so_lan_choi_game) VALUES (%s, %s, %s, %s, %s)",
                           (username, password, ten_hien_thi, diem, so_lan))
            new_id = cursor.lastrowid
            cursor.execute("INSERT IGNORE INTO TienDoHocTap (user_id, ten_bai_hoc) SELECT %s, ten_bai_hoc FROM TienDoHocTap WHERE user_id=%s", (new_id, self.id_guest))
        ...
        except mysql.connector.IntegrityError: return False, "Tên đăng nhập đã tồn tại!"
        finally: conn.close()
