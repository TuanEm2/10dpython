...
class QuanLyHoSo:
    ...
    def dang_ky(self, username, password, ten_hien_thi):
        ...
        conn = self._ket_noi()
        if not conn:
            return False, "Mất kết nối Database!"

        cursor = conn.cursor()
        try:
            # 1. Lấy TOÀN BỘ thông tin (Điểm, Số lần chơi, Thời gian học) của tài khoản Khách
            cursor.execute("SELECT diem_cao_nhat, so_lan_choi_game, thoi_gian_hoc FROM NguoiDung WHERE id=%s",
                           (self.id_guest,))
            row = cursor.fetchone()
            diem, so_lan, thoi_gian = 0, 0, 0.0
            if row:
                diem, so_lan, thoi_gian = row[0], row[1], row[2] if row[2] else 0.0

            # 2. Tạo tài khoản chính thức mới, kế thừa toàn bộ thành tựu trên
            cursor.execute(
            ...
            # 3. Kế thừa Tiến độ học tập & Thống kê lỗi sai sang ID mới
            cursor.execute(
            ...
            # 4. DỌN DẸP TÀI KHOẢN KHÁCH: Trả về trạng thái trắng tinh cho phiên sau
            ...
            # 5. Cấp quyền đăng nhập cho tài khoản mới
    ...
    def cap_nhat_sau_khi_choi(self, diem_moi):
        try:
            ...
            print(f"⚠️ Không thể lưu điểm game (Mất kết nối DB): {e}")
    ...
    def cap_nhat_thoi_gian_hoc(self, phut_vua_hoc):
        ...
        try:
            ...
            print(f"⚠️ Không thể lưu thời gian học (Mất kết nối DB): {e}")

    def ghi_nhan_loi_sai(self, tu_sai):
        # 1. BỘ LỌC CHỐNG RÁC VÀ TỪ KHÓA HỆ THỐNG
        danh_sach_rac = ["...", "?", "", "ERR", "SPACE"]
        if not tu_sai or tu_sai in danh_sach_rac:
            return
        if "HOLDING" in tu_sai or "REC" in tu_sai or "Sai" in tu_sai or "[" in tu_sai:
            return

        # 2. LOGIC GHI NHẬN CŨ (Đã bọc try-except)
        try:
            conn = self._ket_noi()
            if conn:
                cursor = conn.cursor()
             ...
             print(f"⚠️ Không thể ghi nhận lỗi sai (Mất kết nối DB): {e}")
