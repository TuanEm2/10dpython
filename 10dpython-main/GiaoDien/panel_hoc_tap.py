...
class PanelHocTap(ctk.CTkFrame):
    ...
    def chon_bai_hoc(self, ten_bai):
        ...
        if os.path.exists(duong_dan_anh):
            # KÝ GỬI ẢNH VÀO self ĐỂ CHỐNG DỌN RÁC
    ...
    def load_cau_hoi_flashcard(self):
        ...
        if d_anh:
            # KÝ GỬI ẢNH FLASHCARD VÀO self ĐỂ CHỐNG DỌN RÁC
        ...
        if self.loai_cau_hoi == 1:
            ...
            self.lbl_tieu_de_phai.configure(text="Ảnh gợi ý (Ghi nhớ)")
        else:
            ...
            for da in dap_an: ctk.CTkButton(self.khung_trac_nghiem, text=da, font=self.main_app.font_chu_dam, width=60,
                                            height=40, command=lambda ans=da: self.kiem_tra_trac_nghiem(ans)).pack(
                side="left", padx=10)
