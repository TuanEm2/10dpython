...
KY_HIEU_DONG = ['J', 'Z']
SO_FRAME = 30  # 30 khung hình liên tiếp cho 1 cử chỉ (~1 giây)
SO_MAU_MOI_KY_HIEU = 200  # Đảm bảo đủ dữ liệu mẫu cho Deep Learning
    ...
    while True:
        ...
        if dang_ghi:
            if len(toa_do_hien_tai) == 63:
                buffer_ghi.append(toa_do_hien_tai)
                so_frame_da_ghi += 1
            else:
                print("  [HỦY LẦN GHI] Mất dấu tay giữa chừng! Hãy giữ tay ổn định.")
                dang_ghi = False
                buffer_ghi = []
                so_frame_da_ghi = 0
