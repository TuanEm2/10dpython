...
class UngDungNhanDien(ctk.CTk):
    ...
    def dong_ung_dung(self):
        # ÉP HỆ THỐNG LƯU THỜI GIAN TRƯỚC KHI ĐÓNG APP
    ...
    def cap_nhat_giao_dien(self):
        if self.camera_running and self.latest_frame is not None:
            try:
                ...
                # --- TRÓI ẢNH VÀO BỘ NHỚ ---
                self.anh_camera_giu_lai = img

                chu, tin_cay, nguon = self.latest_prediction

                # Chuyển ảnh đã được giữ (self.anh_camera_giu_lai) xuống các panel
                if self.trang_hien_tai == "Học Tập":
                    self.panel_hoc_tap.cap_nhat_khung_hinh(self.anh_camera_giu_lai, chu, tin_cay)
                elif self.trang_hien_tai == "Thực Hành Tự Do":
                    self.panel_thuc_hanh.cap_nhat_khung_hinh(self.anh_camera_giu_lai, chu, tin_cay, nguon)
                elif self.trang_hien_tai == "Minigame":
                    self.panel_tro_choi.cap_nhat_khung_hinh(self.anh_camera_giu_lai, chu, tin_cay, nguon)
            except Exception as e:
