...
from XuLyLogic.engine_nhan_dien import BoXuLyNhanDien
...
class AppHocNgonNguKyHieu(ctk.CTk):
    def __init__(self):
        ...
        # Khởi tạo lõi AI
        self.engine = BoXuLyNhanDien(duong_dan_model='KhoDuLieu/model_tinh.pkl')
    ...
    def tao_man_hinh_bai_hoc(self):
        ...
        danh_sach_bai = ["Bài 1: Chữ A", "Bài 2: Chữ B", "Bài 3: Chữ C"]
    ...
    def cap_nhat_khung_hinh(self):
        ...
        if self.camera_dang_chay and self.cap.isOpened():
            ...
            if ret:
                ...
                # Ép khung hình qua Engine để vẽ xương và dự đoán chữ
                frame, chu_ai_doan, do_tin_cay = self.engine.xu_ly_frame(frame)

                # Cập nhật chữ và % lên giao diện
                phan_tram = int(do_tin_cay * 100)
                self.label_ten_bai.configure(text=f"AI đang thấy chữ: {chu_ai_doan} ({phan_tram}%)")

                # Chuyển hệ màu để hiển thị lên App
            ...
            # Gọi lại hàm này sau 15 mili-giây
